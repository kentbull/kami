...
import kami
from kami.base import filing
from kami.help import helping
from kami.kering import MaxON
...
class LMDBer(filing.Filer):
    ...
    def reopen(self, readonly=False, **kwa) -> bool:
        """
        ...
        with self.env.begin(db=db, write=True, buffers=True) as txn:
            try:
                ...
                raise KeyError(f"Key: `{key}` is either empty, too big (for lmdb),"
                               f" or wrong DUPFIXED size. ref) lmdb.BadValsizeError. Error: {str(ex)}")
