...
from ..core import coring, indexing, wiring
from ..core.http.serving import StaticSink
...
def loadEnds(app, hby, *, default=None, static=False):
    """
    ...
    """
    if static:
        # TODO replace static sink with
        sink = StaticSink(staticDirPath=STATIC_DIR_PATH)
        app.add_sink(sink, prefix=sink.DefaultStaticSinkBasePath)

    # Resources are represented by long-lived class instances
    # handles all requests to '/end' URL path
    app.add_route('/end/{aid}/{role}', PointEnd(hby=hby))
    # handles all requests to '/loc' URL path
    app.add_route('/loc', LocationEnd(hby=hby))
    # handles all requests to '/admin' URL path
    app.add_route('/admin', AdminEnd(hby=hby))
...
def setup(name="who", temp=False, isith=None, count=1,
    ...
    hbyDoer = habbing.HaberyDoer(habery=hby)  # setup doer. # noqa: F841
    ...
    wireDoer = wiring.WireLogDoer(wl=wl)  # noqa: F841
    ...
    # TODO return async functions for habery, wire log, and web server
