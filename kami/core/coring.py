...
from kami.help.helping import codeB64ToB2, intToB64, Reb64, b64ToInt, sceil, nabSextets, \
    codeB2ToB64, nonStringSequence
from kami.kering import Version, deversify, Serials, Rever, versify, DeserializeError, \
    ...
    MAXVERFULLSPAN, InvalidValueError, Ilks, DerivationError, InvalidVarIndexError, smell, \
    VersionError, MaxON, ValidationError, TraitDex, Protocols, Vrsn_2_0, Versionage
...
def loads(raw, size=None, kind=Serials.json):
    ...
    if kind == Serials.json:
        try:
            ...
            raise DeserializeError(f"Error deserializing JSON: {raw[:size].decode("utf-8")}")

    elif kind == Serials.mgpk:
        try:
            ked = msgpack.loads(raw[:size])
        except Exception:
            raise DeserializeError(f"Error deserializing MGPK: {raw[:size]}")

    elif kind == Serials.cbor:
        try:
            ked = cbor.loads(raw[:size])
        except Exception:
            raise DeserializeError(f"Error deserializing CBOR: {raw[:size]}")
...
class MatterCodex:
    ...
    Blind:                str = 'Z'  # Blinding factor 256 bits, Cryptographic strength deterministically generated from random salt  # noqa: E501
    ...
    Ed448N:               str = '1AAC'  # Ed448 non-transferable prefix public signing verification key. Basic derivation.  # noqa: E501
...
class Matter:
        ...
        if raw is not None:  # raw provided but may be empty
            if not code:
                raise EmptyMaterialError("Improper initialization need either "
                                         "(raw not None and code) or "
                                         "(code and soft) or "
                                         "qb64b or qb64 or qb2.")
    ...
    def _exfil(self, qb64b):
        ...
        Parameters:
            qb64b (str | bytes | memoryview): fully qualified base64 from stream
...
class Tagger(Matter):
    ...
    def __init__(self, tag='', soft='', code=None, **kwa):
        ...
        if tag:
            ...
            l = len(tag)  # soft not empty so l > 0  # noqa: E741
            if l > len(codes):
...
class Counter:
    ...
    def countToB64(self, l=None):  # noqa: E741
...
class Tholder:
    ...
    def weight(w: str) -> Fraction:
        ...
        except ValueError:  # not float str or int str so try ration str
            w = Fraction(w)
