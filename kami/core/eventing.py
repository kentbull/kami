...
# not used should this be deprecated
...
def state(pre,
    ...
    if not eevt or not isinstance(eevt, StateEstEvent):
        raise ValueError(f"Missing or invalid latest est event = {eevt} for key "
                         f"state.")
    eesner = Number(numh=eevt.s)  # if not whole number raises InvalidValueError  # noqa: F841
...
def reply(route="",
    ...
    """
    label = coring.Saids.d    # noqa: F841
    vs = versify(version=version, kind=kind, size=0)
    if data is None:
        data = {}
...
class Kever:
    ...
    def validateDelegation(self, serder, sigers, wigers, wits, local=True,
        ...
        if delseqner is None or delsaider is None:
            if self.locallyOwned(delpre):  # local delegator so escrow.
                ...
                raise MissingDelegationError(
                    f"No delegation seal for delegator {delpre} of evt = {serder.ked}.")
        ...
        if raw is None:  # no delegating event at key pre, sn
            ...
            sn = Number(num=serder.sn).validate(inceptive=inceptive).sn  # noqa: F841
            self.escrowPSEvent(serder=serder, sigers=sigers, wigers=wigers, local=local)
            self.escrowPACouple(serder=serder, seqner=delseqner, saider=delsaider)
    ...
    def escrowPSEvent(self, serder, sigers, wigers=None, local=True):
        ...
        # b'EOWwyMU3XA7RtWdelFt-6waurOTH_aW_Z9VTaU-CshGk.00000000000000000000000000000001'
...
class Kevery:
    ...
    def duplicity(self, serder, sigers):
        ...
        Handles duplicity detection and logs if duplicitous
