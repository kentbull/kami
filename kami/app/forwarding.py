...
from kami.app import agenting
from kami.core import coring, eventing
from kami.db import dbing
from kami.help import decking, ogler
from kami.kering import Roles
from kami.peer import exchanging

logger = ogler.getLogger()


class Poster:  # used to be a (doing.DoDoer)
    pass
    # """
    ...
    #                                     hab, ends[role], recp=recp, serder=srdr, atc=atc, topic=tpc)
    #                             else:
    #                                 yield from self.sendDirect(
    #                                     hab, ends[role], serder=srdr, atc=atc)
    #
    #                 # otherwise send to one witness
    #                 elif Roles.witness in ends:
    #                     yield from self.forwardToWitness(
    #                         hab, ends[Roles.witness], recp=recp, serder=srdr, atc=atc, topic=tpc)
    #                 else:
...
class StreamPoster:
    ...
    def __init__(self, hby, recp, src=None, hab=None, mbx=None, topic=None, headers=None, **kwa):
        ...
        self.recp = recp
        self.src = src
        self.messagers = []  # TODO likely a list of Doers or DoDoers
        self.mbx = mbx
        self.topic = topic
