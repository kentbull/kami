...
from kami import help
from kami.app import habbing, keeping, configing
from kami.app.configing import ConfigerCtx
from kami.app.habbing import HaberyCtx, MakeHaberyCtx, HabCtx
from kami.app.keeping import KeeperCtx
from kami.db import basing
from kami.core import coring, eventing, parsing
from kami.db.basing import BaserCtx
from kami.help import helping


@pytest.mark.asyncio
async def test_habery():
    """
    Test Habery class
    """
    # test default
    ...
    async with (AsyncExitStack() as stack):
        cf = await stack.enter_async_context(
            # not opened by default, opens
            ConfigerCtx(configer=configing.Configer(name=name, base=base, temp=temp)))
        ks = await stack.enter_async_context(
            # not opened by default, opens
            KeeperCtx(keeper=keeping.Keeper(name=base, temp=temp)))
        db = await stack.enter_async_context(
            # not opened by default, opens
            BaserCtx(baser=basing.Baser(name=base, temp=temp)))
        ...
        if not conf:  # setup config file
            ...
            # setup habery
            async with HaberyCtx(habery=habbing.Habery(
                    name=name, base=base, ks=ks, db=db, cf=cf, temp=temp, bran=bran, salt=default_salt)
            ) as hby:
...
@pytest.mark.asyncio
async def test_pre_create_habery():
    default_salt = coring.Salter(raw=b'0123456789abcdef').qb64
    seed4bran = 'ALQVu9AjGW3JrIzX0UHm2awGCbDXcsLzy-vAE649Fz1j'
    aeid4seed = 'BHRYV_5a1AlibCrXFG_KDD9rC6aXx9cb0sR968NL80VI'
    ...
    async with MakeHaberyCtx(
            name=name, base=base, temp=temp, bran=bran, salt=default_salt, free=True
    ) as hby:
    ...
    async with MakeHaberyCtx(salt=coring.Salter(raw=b'0123456789abcdef').qb64) as hby:
        assert hby.name == "test"
        assert hby.base == ""
        assert hby.temp
        assert hby.inited
    ...
    bran = "MyPasscodeARealSecret"
    async with MakeHaberyCtx(bran=bran, salt=coring.Salter(raw=b'0123456789abcdef').qb64) as hby:
        assert hby.name == "test"
        assert hby.base == ""
        assert hby.temp
        assert hby.inited
...
@pytest.mark.asyncio
async def test_make_load_hab_with_habery():
    """
    ...
    async with MakeHaberyCtx(salt=coring.Salter(raw=b'0123456789abcdef').qb64) as hby:  # default is temp=True on openHab
    ...
    async with MakeHaberyCtx(base=base, temp=False, salt=coring.Salter(raw=b'0123456789abcdef').qb64) as hby:  # default is temp=True
    ...
    base = "hold"
    async with MakeHaberyCtx(base=base, temp=False) as hby:  # default is temp=True
        assert hby.cf.path.endswith("keri/cf/hold/test.json")
        assert hby.db.path.endswith("keri/db/hold/test")
        assert hby.ks.path.endswith('keri/ks/hold/test')
...
@pytest.mark.asyncio
async def test_hab_rotate_with_witness():
    """
    ...
    async with MakeHaberyCtx(name=name, base="test", temp=False) as hby:
...
@pytest.mark.asyncio
async def test_habery_reinitialization():
    ...
    async with MakeHaberyCtx(name=name, base="test", temp=False, clear=True) as hby:
...
@pytest.mark.asyncio
async def test_habery_signatory():
    async with MakeHaberyCtx(salt=coring.Salter(raw=b'0123456789abcdef').qb64) as hby:
        signer = hby.signator
...
@pytest.mark.asyncio
async def test_habery_reconfigure(mockHelpingNowUTC):
    """
    ...
    async with (
        MakeHaberyCtx(name='wes', base=cbase, salt=salt) as wesHby,
        MakeHaberyCtx(name='wok', base=cbase, salt=salt) as wokHby,
        MakeHaberyCtx(name=cname, base=cbase, salt=salt) as tamHby,
        MakeHaberyCtx(name='wat', base=cbase, salt=salt) as watHby,
        MakeHaberyCtx(name=pname, base=pbase, salt=salt) as nelHby):
...
@pytest.mark.asyncio
async def test_namespaced_habs():
    async with MakeHaberyCtx(salt=coring.Salter(raw=b'0123456789abcdef').qb64) as hby:
...
@pytest.mark.asyncio
async def test_make_other_event():
    async with MakeHaberyCtx(salt=coring.Salter(raw=b'0123456789abcdef').qb64) as hby:
        hab = hby.makeHab(name="test")
        assert hab.pre == "EIaGMMWJFPmtXznY1IIiKDIrg-vIyge6mBl2QV8dDjI3"
...
@pytest.mark.asyncio
async def test_hab_by_pre():
    async with MakeHaberyCtx() as hby:
...
@pytest.mark.asyncio
async def test_postman_endsfor():
    async with MakeHaberyCtx(name="test", temp=True, salt=coring.Salter(raw=b'0123456789abcdef').qb64) as hby, \
            MakeHaberyCtx(name="wes", temp=True, salt=coring.Salter(raw=b'wess-the-witness').qb64) as wesHby, \
            HabCtx(name="agent", temp=True, salt=b'0123456789abcdef') as (agentHby, agentHab):
